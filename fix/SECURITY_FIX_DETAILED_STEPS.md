# SeaTunnel 安全修复 — 逐步详细变更记录

> **基准**：Apache SeaTunnel **2.3.13** 官方源码
> **扫描**：第一轮 `SCA_ScanReport/Vulnerabilities.csv`（113 Critical）；第二轮 `SCA_ScanReport_V2/Vulnerabilities.csv`（25 Critical）
> **策略**：优先升级至 LatestFixedVersion；API / Java 8 / shade 不兼容时选可编译的最高安全版本
> **文档生成**：2026-06-30
> **说明**：本文档记录每一步操作、每一个文件、每一处修改的前后对照。
...
## 补充：Java 源码逐步修改（第二轮 Spark 3.5.8）

### 步骤 A — SeaTunnelBatchWrite.java

**路径**：`seatunnel-translation/.../SeaTunnelBatchWrite.java`

| 位置 | 修改前 | 修改后 |
|------|--------|--------|
| abort 与 combineCommitMessage 之间 | 无此方法 | 新增 `useCommitCoordinator()` 返回 `true` |

### 步骤 B — TransformExecuteProcessor.java

**路径**：`seatunnel-spark-starter-common/.../TransformExecuteProcessor.java`

| 位置 | 修改前 | 修改后 |
|------|--------|--------|
| import | `org.apache.spark.sql.catalyst.encoders.RowEncoder` | `Encoder` + `Encoders` |
| sparkTransform | `RowEncoder.apply(outputManager.getTableSchema())` | `Encoders.row(outputManager.getTableSchema())` |
...
构建通过后执行 `mvn clean` 并删除：`seatunnel-engine-ui` 下 `node/`、`node_modules/`、`dist/`、`.deployed`；`seatunnel-engine-server/.../resources/ui/` 静态资源；根目录 `build-*.log`。

---

*本文档为逐步、逐文件、逐处前后对照的完整记录。*
